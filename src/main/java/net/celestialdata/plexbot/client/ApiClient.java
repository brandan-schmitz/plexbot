...
import net.celestialdata.plexbot.client.auth.ApiKeyAuth;
import net.celestialdata.plexbot.client.auth.Authentication;
import net.celestialdata.plexbot.client.auth.HttpBasicAuth;
import net.celestialdata.plexbot.client.auth.HttpBearerAuth;
import okhttp3.*;
...
public class ApiClient {
    ...
    private String basePath = "https://api.real-debrid.com/rest/1.0";
    ...
    public ApiClient() {
        ...
        authentications.put("omdbApiKey", new ApiKeyAuth("query", "apikey"));
        authentications.put("plexAuth", new HttpBasicAuth());
        authentications.put("plexAuthToken",
                new ApiKeyAuth("header", "X-Plex-Token"));
    ...
    public ApiClient setHttpClient(OkHttpClient newHttpClient) {
        this.httpClient =
                Objects.requireNonNull(newHttpClient, "HttpClient must not be null!");
        return this;
    }
    ...
    /**
     * Helper method to set the username and password for the plex.tv account.
     *
     * @param username Username
     * @param password Password
     */
    public void setPlexLogin(String username, String password) {
        var auth = authentications.get("plexAuth");
        if (auth instanceof HttpBasicAuth) {
            ((HttpBasicAuth) auth).setUsername(username);
            ((HttpBasicAuth) auth).setPassword(password);
    ...
    /**
     * Helper method to set Real-Debrid Bearer Token.
     *
     * @param token bearer token
     */
    public void setRdbBearerToken(String token) {
        var auth = authentications.get("rdbApiKey");
        if (auth instanceof HttpBearerAuth) {
    ...
    /**
     * Helper method to set OMDb API key value.
     *
     * @param apiKey API Key
     */
    public void setOmdbApiKey(String apiKey) {
        var auth = authentications.get("omdbApiKey");
        if (auth instanceof ApiKeyAuth) {
            ((ApiKeyAuth) auth).setApiKey(apiKey);
        } else throw new RuntimeException("Invalid Auth Type");
    }
    ...
    /**
     * Helper method to set Plex Authentication Token value.
     *
     * @param token Authentication Token
     */
    public void setPlexAuthToken(String token) {
        var auth = authentications.get("plexAuthToken");
        if (auth instanceof ApiKeyAuth) {
            ((ApiKeyAuth) auth).setApiKey(token);
        } else throw new RuntimeException("Invalid Auth Type");
    }

    /**
     * Helper method to set the Plex Client Identifier value.
     *
     * @param clientIdentifier Client Identifier
     */
    public void setPlexClientIdentifier(String clientIdentifier) {
    ...
    /**
     * Formats the specified query parameter to a list containing a single {@code
     * Pair} object.
     * <p>
    ...
    public File prepareDownloadFile(Response response) throws IOException {
        ...
        if (filename == null) {
            ...
            // File.createTempFile requires the prefix to be at least three characters
            // long
            if (prefix.length() < 3)
                prefix = "download-";
        }
    ...
    public <T> ApiResponse<T> execute(Call call, Type returnType)
        ...
        try {
            ...
            return new ApiResponse<>(response.code(),
                    response.headers().toMultimap(), data);
        } catch (IOException e) {
            throw new ApiException(e);
        }
    }
    ...
    public <T> void executeAsync(Call call, final Type returnType,
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                ...
                callback.onSuccess(result, response.code(),
                        response.headers().toMultimap());
            }
        });
    }
    ...
    public Call buildCall(String path, String method, List<Pair> queryParams,
        ...
        Request request = buildRequest(
                path, method, queryParams, collectionQueryParams, body, headerParams,
    ...
    public Request
    buildRequest(String path, String method, List<Pair> queryParams,
                 List<Pair> collectionQueryParams, Object body,
                 Map<String, String> headerParams,
        ...
        if (callback != null && reqBody != null) {
            ProgressRequestBody progressRequestBody =
                    new ProgressRequestBody(reqBody, callback);
        ...
        try {
            ...
            if (!verifyingSsl) {
                ...
                if (sslCaCert == null) {
                    ...
                    @SuppressWarnings("ConstantConditions") KeyStore caKeyStore = newEmptyKeyStore(password);
                    int index = 0;
